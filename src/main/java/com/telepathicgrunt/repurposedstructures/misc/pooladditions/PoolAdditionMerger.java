        ...
        for(StructurePoolElement element : elements) {
            if(element instanceof SinglePoolElement singlePoolElement) {
                ...
                if(structureTemplate.isEmpty()) {
                    RepurposedStructuresApi.LOGGER.error("(Repurposed Structures POOL MERGER) Found an entry in {} that points to the non-existent nbt file called {}", feedingPool, nbtID.get());
                }
            }
            ...
            super(fallback, rawTemplatesWithConditions.stream().filter(triple -> {
                if(triple.condition().isPresent()) {
                    ...
                    if(optionalSupplier.isPresent()) {
                        ...
                        RepurposedStructuresApi.LOGGER.error("Repurposed Structures Error: Found {} entry has a condition that does not exist. Extra info: {}", name, fallback);
                    }
                }
                return true;
